...
mod tests {
    ...
    #[test]
    fn task_output_empty() {
        let output = TaskOutput::empty(Duration::from_millis(50));
        assert_eq!(output.result, serde_json::Value::Null);
    }

    #[test]
    fn task_context_with_parent() {
    ...
    fn task_job_with_id() {
        let id = Uuid::new_v4();
        let task = Task::job_with_id(id, "Test", "Desc");
    ...
    fn task_tool_exec() {
        ...
        let task = Task::tool_exec(parent_id, "echo", serde_json::json!({"msg": "hi"}));
    ...
    fn task_debug_format() {
        let task = Task::job("Debug Test", "Description");
        let debug = format!("{:?}", task);
        assert!(debug.contains("Task::Job"));
