...
use crate::agent::routine::{
    next_cron_fire, NotifyConfig, Routine, RoutineAction, RoutineGuardrails, Trigger,
};
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "schedule": {
                    "type": "string",
                    "description": "Cron expression (for cron trigger), 6-field: sec min hour day month weekday"
                },
                "event_pattern": {
                    "type": "string",
                ...
                "event_channel": {
                    "type": "string",
                    "description": "Optional channel filter for event trigger"
                },
                "prompt": {
                    "type": "string",
                ...
                "context_paths": {
                    "type": "array",
                    "items": { "type": "string" },
                    "description": "Workspace paths to load as context"
                },
                "action_type": {
                    "type": "string",
                    "enum": ["lightweight", "full_job"],
                    "description": "lightweight (single LLM call, default) or full_job (multi-turn)"
                },
                "cooldown_secs": {
                    "type": "integer",
...
impl Tool for RoutineListTool {
    ...
    async fn execute(
        ...
        Ok(ToolOutput::success(
            serde_json::json!({ "count": list.len(), "routines": list }),
            start.elapsed(),
        ))
    }
}
...
impl Tool for RoutineUpdateTool {
    ...
    fn description(&self) -> &str {
        "Update an existing routine. Pass the routine name and only the fields to change."
    }
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
            "properties": {
                "name": { "type": "string", "description": "Name of the routine to update" },
...
impl Tool for RoutineDeleteTool {
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
            "properties": {
                "name": { "type": "string", "description": "Name of the routine to delete" }
            },
            "required": ["name"]
        })
    }
...
impl Tool for RoutineHistoryTool {
    ...
    fn description(&self) -> &str {
        "View the execution history of a routine with status, duration, and results."
    }
...
mod tests {
    ...
    fn routine_create_schema_has_required_fields() {
        // Verify the JSON schema structure matches expectations
        ...
        assert!(required.contains(&"trigger_type".into()));
        assert!(required.contains(&"prompt".into()));
    }

    #[tokio::test]
    async fn routine_list_empty() {
        let db = test_db().await;
        let tool = RoutineListTool::new(db);
    ...
    #[tokio::test]
    async fn routine_history_not_found() {
        let db = test_db().await;
        let tool = RoutineHistoryTool::new(db);
        let ctx = JobContext::default();
