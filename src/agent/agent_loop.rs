...
pub struct AgentDeps {
    ...
    pub reply_drafter: Option<Arc<ReplyDrafter>>,
...
impl Agent {
    ...
    pub(crate) fn reply_drafter(&self) -> Option<&Arc<ReplyDrafter>> {
        self.deps.reply_drafter.as_ref()
    }
    ...
    async fn process_user_input(
        ...
        // NOTE: Email no longer flows through agent_loop — email metadata extraction removed.
        // Email cards are generated by the standalone pipeline (email_processor → MessageProcessor).
        if let (Some(drafter), Some(queue)) =
            (self.reply_drafter().cloned(), self.deps.card_queue.clone())
        {
            ...
            let expire = drafter.expire_minutes();
            tokio::spawn(async move {
                match drafter.draft(&msg_content, &sender, &chat_id).await {
                    Ok(Some(draft)) => {
                        let card = ApprovalCard::new(
                            CardPayload::Reply {
                                channel,
                                source_sender: sender,
                                source_message: msg_content,
                    ...
                    Err(e) => tracing::warn!("Reply drafting failed: {}", e),
                }
            });
        }
