...
mod tests {
    ...
    #[test]
    fn floor_char_boundary_at_valid() {
    ...
    fn signals_completion_no_bare_substrings() {
        ...
        assert!(!llm_signals_completion("Function done_callback was called."));
        assert!(!llm_signals_completion("Set is_complete = true"));
        assert!(!llm_signals_completion("Running step 3 of 5"));
        assert!(!llm_signals_completion("I need to complete more work first."));
        assert!(!llm_signals_completion("Let me finish the remaining steps."));
        assert!(!llm_signals_completion("I'm done analyzing, now let me fix it."));
        assert!(!llm_signals_completion("I completed step 1 but step 2 remains."));
    }
    ...
    fn signals_completion_tool_output_injection() {
        ...
        assert!(!llm_signals_completion("The tool returned: TASK_COMPLETE signal"));
    }
}
