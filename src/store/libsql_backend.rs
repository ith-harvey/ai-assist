    ...
    async fn upsert_calendar_sync_state(
        ...
        conn.execute(
            ...
            params![
                ...
                state.calendar_id,
                state.calendar_name,
                opt_text(state.sync_token.as_deref()),
                opt_text_owned(state.last_sync_at.map(|d| d.to_rfc3339())),
                state.sync_status.to_string(),
                opt_text(state.error_message.as_deref()),
    ...
    async fn upsert_calendar_event(
        ...
        conn.execute(
            ...
            params![
                ...
                opt_text(event.location.as_deref()),
                opt_text(event.description.as_deref()),
                attendees_json,
                opt_text(event.color_id.as_deref()),
                opt_text(event.etag.as_deref()),
                opt_text_owned(event.google_updated_at.map(|d| d.to_rfc3339())),
